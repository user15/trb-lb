...
/* Searches for maximum item in |tree| less than or equal to |item|.
   If found, initializes |trav| to the item found and returns the item
   as well.
   If there is no matching item, initializes |trav| to the null item
   and returns |NULL|. */
void *
trb_t_lower_bound (struct trb_traverser *trav, struct trb_table *tree, void *item)
{
  struct trb_node *p, *lb = NULL;
